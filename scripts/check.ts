...
for (const s of SECTIONS) {
  ...
  // ---- 節の厚み ----
  // **`00-common.md` は「短すぎる節は npm run check が警告します（800 字未満）」と
  // 書いていましたが、その検査はありませんでした**（2026 年 9 月 20 日に 9 本目で気づいた）。
  // プロンプトが道具について嘘をついていたことになります。書き手はプロンプトを信じるので、
  // 「検査されるなら鳴らなければよい」と考えて下限すれすれを狙う理由になります。
  // 800 ではなく 2,000 で切っているのは、800 字未満の節が 1 つも無く（最小 1,451 字）、
  // その値では何も捕まらないからです。**何も捕まらない床は、床ではありません。**
  if (body.length < 2000) {
