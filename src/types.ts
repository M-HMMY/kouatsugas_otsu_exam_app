 ...
 * 前に範囲外の入門編を置く。
 *
 * **★ 3 科目なのに分野が 5 つあるのは、乙種化学と乙種機械を 1 つのアプリに入れているから。**
 * 受験者はどちらか一方しか受けないので、模試は区分を選んでから作る。
 * 詳しくは `src/data/categories.ts` の注記を読むこと。
 *
 * **★ 区分で分かれるのは学識だけ**（令和 7 年度の公開問題で確認。
 * `docs/public-questions.md` §1）。
 *
 * | 分野 | 区分による違い |
 * | --- | --- |
 * | `law` 20 問 | **完全に同一。**1 本で足りる |
 * | `hoan` 15 問 | **15 問中 14 問が同一。**1 本で足りる（違うのは本番の問 7 だけ） |
 * | `gaku-kagaku` / `gaku-kikai` 15 問 | **完全に別。**ここだけ 2 本持つ |
 *
 * **以前は保安管理技術も `hoan-kagaku` / `hoan-kikai` に割っていたが、
 * 同じ本文を 2 回書くことになるので 1 つにまとめた**（`docs/section-plan.md` §1。
 * 戻し方もそこに書いてある）。
 */
export type FieldId = 'intro' | 'law' | 'hoan' | 'gaku-kagaku' | 'gaku-kikai';
...
export interface Category {
  ...
  syllabus: string;
  /** 本番のおおよその出題数の推定値。KHK は章ごとの内訳を公表していない（categories.ts の注記を参照） */
  questions: number;
  /**
 ...
 *
 * 本番は 1 区分 50 問 / 270 分なので 1 問あたり 5 分 24 秒あり、姉妹アプリより時間に余裕がある
 * （法令 20 問 / 60 分、保安管理技術 15 問 / 90 分、学識 15 問 / 120 分）。
 * ただし**法令の条文を読ませる問題は長くなりがち**なので、長さの上限は別に決めてある
 * （`scripts/check.ts` を参照）。
 *
 * **設問は「イ・ロ・ハ（・ニ）の記述のうち正しいものの組合せ」の形で書く。**
 * 記述は `question` に並べ、`choices` には `イ、ハ` のような組合せを入れる。
 * 詳しくは `src/data/questions/index.ts` の注記と `docs/public-questions.md` §2 を読むこと。
 *
