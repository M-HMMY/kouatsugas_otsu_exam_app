 ...
 * **法令の問題に必ず置く前置き。**
 *
 * 令和 7 年度の公開問題では、法令の問 1 の前にこの 3 つが宣言されていた
 * （`docs/public-questions.md`「法令の問題文に必ず置かれている前置き」）。
 * **同文書が「自前の問題でも同じ前置きを置くこと」と書いている。**
 *
 * **★ これは飾りではない。**置かないと、記述の正誤が決まらなくなる問題がある。
 * 例えば容器への充塡の要件（法 48 条 1 項・2 項）は、
 * **同条 5 項で「経済産業大臣が危険のおそれがないと認め、条件を付して許可した場合」に
 * 適用しないとされている。**2 つめの宣言がないと、
 * 「充塡する容器は刻印等がされているものでなければならない」という記述を
 * 「例外なく正しい」とは言えない。**同じ形の例外が法令のあちこちにある。**
 *
 * 1 つめの日付は**基準日**で、受験案内書が「令和 8 年 4 月 1 日現在施行されている
 * 法令に基づき出題する」としている（CLAUDE.md の検証観点 2）。
 * **年度が変わったら、ここも取り直すこと。**
 */
export const LAW_PREAMBLE: readonly string[] = [
  '令和 8 年 4 月 1 日現在施行されている高圧ガス保安法に係る法令に基づいて出題しています。',
  '経済産業大臣が危険のおそれがないと認めた場合等における規定は、適用しないものとします。',
  '「都道府県知事等」とは、都道府県知事又は高圧ガス保安法に関する事務を処理する指定都市の長をいいます。',
];
